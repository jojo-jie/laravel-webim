$(function() {
  webim.init();
  webim.data.message = $('#user_id').data('message');
...
var webim = {
  ...
  open : function() {
    this.data.wsServer.onopen = function(evt) {
        var str = {'id':webim.data.message.id,'type':webim.data.message.type};
        var json_str = JSON.stringify(str);
        webim.data.wsServer.send(json_str);
        webim.notice('连接成功');
    }
  },
  close : function() {
